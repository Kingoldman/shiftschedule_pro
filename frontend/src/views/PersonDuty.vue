...
const periodLabel = computed(() => {
  if (statMode.value === "monthly")
    return currentMonth.value.format("YYYY 年 M 月");
...
function customFreq(row) {
  const eligibleMap = {
    workday: "eligible_workday",
    weekend: "eligible_weekend",
    holiday: "eligible_holiday",
  };
  let eligible = 0,
    count = 0;
...
async function loadEmployeeData() {
  ...
  try {
    ...
    if (statMode.value === "monthly") {
      ...
      result = await statsApi.employee(
        selectedId.value,
        "yearly",
        currentYear.value,
      );
...
async function exportPDF() {
  ...
  if (
    !employeeData.value ||
    employeeData.value.overview.total_duty_days === 0
  ) {
  ...
  overlay.style.cssText =
    "position:fixed;inset:0;background:rgba(255,255,255,0.97);z-index:99998;display:flex;align-items:center;justify-content:center;font-size:15px;color:#666;";
  ...
  try {
    ...
    const calcFreqAndFormula = (row, type) => {
      ...
      const eligible = row[eligibleKey[type]] || 0;
      const count = row[countKey[type]] || 0;
      if (!count || !eligible)
        return { freq: "-", formula: `${eligible}天统计 / ${count}天值班` };
      return {
        freq: `每${(eligible / count).toFixed(1)}天`,
        formula: `${eligible}天统计 / ${count}天值班`,
      };
    };

    // 捕获图表为图片
    const trendImg =
      trendChart?.getDataURL({
        type: "png",
        pixelRatio: 2,
        backgroundColor: "#fff",
      }) || "";
    ...
    // 月度明细表
    const trendRows = [...(data.monthly_trend || [])]
      .sort((a, b) => a.month.localeCompare(b.month))
      .map((m) => {
        const wf = calcFreqAndFormula(m, "workday"),
          wef = calcFreqAndFormula(m, "weekend");
    ...
    const allDuties = [...(data.all_duties || [])].sort((a, b) =>
    ...
    const wrapperStyle =
      'font-family:"Microsoft YaHei","PingFang SC",sans-serif;color:#1f2937;padding:16px 20px 36px 20px;width:750px';

    // 各 section 独立 HTML
    const sections = [
      // 1. 报告头 + 基本信息
      `<div style="${wrapperStyle}">
        <div style="text-align:center;border-bottom:2px solid #409eff;padding-bottom:10px;margin-bottom:12px">
          <div style="font-size:11px;color:#6b7280;letter-spacing:2px">SHIFT SCHEDULE SYSTEM</div>
          <div style="font-size:20px;font-weight:bold;color:#337ecc;margin:3px 0">个人值班数据报告</div>
      ...
      // 2. 概览统计卡片
      `<div style="${wrapperStyle}">
        <div style="font-size:14px;font-weight:bold;margin-bottom:8px;border-left:3px solid #409eff;padding-left:8px">概览统计</div>
        <table style="width:100%;font-size:12px;border-collapse:collapse">
          <tr>
            <td style="background:#ecf5ff;padding:8px;text-align:center;border:1px solid #d9ecff"><div style="font-size:10px;color:#6b7280">累计值班</div><div style="font-size:20px;font-weight:bold;color:#337ecc">${ov.total_duty_days}<span style="font-size:11px">天</span></div></td>
            <td style="background:#ecf5ff;padding:8px;text-align:center;border:1px solid #d9ecff"><div style="font-size:10px;color:#6b7280">统计天数</div><div style="font-size:20px;font-weight:bold;color:#337ecc">${ov.eligible_total}<span style="font-size:11px">天</span></div></td>
          ...
          <tr>
            <td style="background:#ecf5ff;padding:8px;text-align:center;border:1px solid #d9ecff"><div style="font-size:10px;color:#6b7280">周末频率</div><div style="font-size:15px;font-weight:bold;color:#409eff">${ov.freq_weekend ? "每" + ov.freq_weekend + "天" : "-"}</div><div style="font-size:9px;color:#9ca3af">${ovFreqFormula("weekend")}</div></td>
    ...
    for (let ci = 0; ci < dutyTotalPages; ci++) {
      const chunk = allDuties.slice(
        ci * DUTY_CHUNK_SIZE,
        (ci + 1) * DUTY_CHUNK_SIZE,
      );
      const chunkRows = chunk.map(renderDutyRow).join("");
      const pageLabel =
        dutyTotalPages > 1 ? `（第${ci + 1}/${dutyTotalPages}页）` : "";
      sections.push(`<div style="${wrapperStyle}">
    ...
    for (let si = 0; si < sections.length; si++) {
      ...
      sDiv.style.cssText =
        "position:fixed;top:0;left:0;width:750px;background:#ffffff;z-index:99997;";
      ...
      try {
        ...
        const sCanvas = await html2canvas(sDiv, {
          ...html2canvasOpts,
          height: renderHeight,
          windowHeight: renderHeight + 100,
        });
...
watch(statMode, () => {
  if (selectedId.value) loadEmployeeData();
});
watch(currentMonth, () => {
  if (statMode.value === "monthly" && selectedId.value) loadEmployeeData();
});
watch(currentYear, () => {
  if (statMode.value === "yearly" && selectedId.value) loadEmployeeData();
});
...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex items-start justify-between mb-4">
      <div>
        <div
          class="text-xs text-blue-600 font-mono tracking-[0.3em] uppercase mb-1"
        >
          / Personal Query
        </div>
        <h1 class="font-display text-3xl font-semibold text-gray-800">
          个人查询
        </h1>
      ...
      <button
        v-if="
          canEdit &&
          employeeData &&
          employeeData.overview &&
          employeeData.overview.total_duty_days > 0
        "
    ...
    <div class="card p-4 mb-4">
      <div class="flex items-center gap-4 flex-wrap">
          ...
          >
            <div class="flex items-center gap-2">
              <span
                ...
                "
              ></span>
              <span :class="emp.deleted ? 'text-red-500' : ''">{{
                emp.name
              }}</span>
              <span v-if="emp.deleted" class="text-xs text-red-400"
                >已删除</span
              >
              <span v-else-if="emp.state === 0" class="text-xs text-gray-400"
                >不值班</span
              >
        ...
        <template v-if="statMode === 'monthly'">
          ...
          <div class="flex items-center gap-2">
            <button class="btn-ghost px-2 py-1" @click="prevMonth">
              <el-icon><ArrowLeft /></el-icon>
            </button>
            <span
              class="font-display text-lg font-semibold num min-w-[120px] text-center"
            >
              {{ currentMonth.format("YYYY 年 M 月") }}
            </span>
            <button class="btn-ghost px-2 py-1" @click="nextMonth">
              <el-icon><ArrowRight /></el-icon>
            </button>
            <button class="btn-ghost px-2 py-1 text-xs" @click="goThisMonth">
              本月
            </button>
            <el-date-picker
        ...
        <template v-if="statMode === 'yearly'">
          <div class="h-6 w-px bg-gray-200"></div>
          <div class="flex items-center gap-2">
            <button class="btn-ghost px-2 py-1" @click="prevYear">
              <el-icon><ArrowLeft /></el-icon>
            </button>
            <span
              class="font-display text-lg font-semibold num min-w-[80px] text-center"
              >{{ currentYear }} 年</span
            >
            <button class="btn-ghost px-2 py-1" @click="nextYear">
              <el-icon><ArrowRight /></el-icon>
            </button>
            <button class="btn-ghost px-2 py-1 text-xs" @click="goThisYear">
              本年
            </button>
            <el-date-picker
    ...
    <div v-loading="loading">
      <template v-if="employeeData && employeeData.overview">
          ...
          <div class="flex items-center justify-between flex-wrap gap-3">
            <div class="flex items-center gap-3">
              <div>
                <div class="flex items-center gap-2">
                  <span
                    class="font-display text-xl font-semibold text-gray-800"
                    >{{ employeeData.employee?.name }}</span
                  >
                  <span
                    v-if="employeeData.employee?.state === 1"
                    class="day-badge day-badge-workday"
                    >值班中</span
                  >
                  <span
                    v-else-if="employeeData.employee?.state === 0"
                    class="day-badge bg-gray-100 text-gray-500"
                    >不值班</span
                  >
                  <span v-else class="day-badge bg-gray-100 text-gray-400"
                    >已删除</span
                  >
                </div>
                <div class="text-xs text-gray-500 mt-1">
                  所属组：<span class="font-medium text-gray-700">{{
                    employeeData.employee?.group_name || "未分组"
                  }}</span>
                  <span class="mx-2 text-gray-300">|</span>
                  共
                  <span class="num font-semibold text-gray-700">{{
                    employeeData.duty_days?.length || 0
                  }}</span>
                  天值班
                </div>
              </div>
        ...
        <template v-if="employeeData.overview.total_duty_days > 0">
          <!-- 概览数字卡片 -->
          <div
            class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4"
          >
            <div class="card p-4">
              <div class="text-xs text-gray-500 mb-1">累计值班</div>
              <div
                class="font-display text-2xl font-semibold num text-gray-800"
              >
            ...
            <div class="card p-4">
              ...
              <div
                class="font-display text-2xl font-semibold num text-blue-600"
              >
            ...
            <div class="card p-4">
              ...
              <div
                class="font-display text-2xl font-semibold num text-green-600"
              >
            ...
            <div class="card p-4">
              ...
              <div
                class="font-display text-2xl font-semibold num text-amber-600"
              >
          ...
          <div class="card mb-4">
            <div
              class="px-5 py-4 border-b border-gray-200 flex items-center justify-between flex-wrap gap-3"
            >
            ...
            >
              <el-table-column prop="month" width="100">
                <template #header>
                  <span
                    ...
                    :class="sortHeaderClass('month')"
                    @click="toggleSort('month')"
                    >月份 <span v-html="sortIconHtml('month')"></span
                  ></span>
                </template>
              ...
              <el-table-column align="center">
                <template #header>
                  <span
                    class="cursor-pointer select-none"
                    :class="sortHeaderClass('workday')"
                    @click="toggleSort('workday')"
                    >工作日(含调休补班)
                    <span v-html="sortIconHtml('workday')"></span
                  ></span>
                </template>
                <template #default="{ row }">
                  <span
              ...
              <el-table-column align="center">
                <template #header>
                  <span
                    class="cursor-pointer select-none"
                    :class="sortHeaderClass('weekend')"
                    @click="toggleSort('weekend')"
                    >周末 <span v-html="sortIconHtml('weekend')"></span
                  ></span>
                </template>
                <template #default="{ row }">
                  <span
                    class="num"
                    :class="
                      sortKey === 'weekend' ? 'font-semibold text-blue-600' : ''
                    "
                    >{{ row.weekend || 0 }}</span
                  >
                </template>
              ...
              <el-table-column align="center">
                <template #header>
                  <span
                    class="cursor-pointer select-none"
                    :class="sortHeaderClass('holiday')"
                    @click="toggleSort('holiday')"
                    >节假日 <span v-html="sortIconHtml('holiday')"></span
                  ></span>
                </template>
                <template #default="{ row }">
                  <span
                    class="num"
                    :class="
                      sortKey === 'holiday' ? 'font-semibold text-red-500' : ''
                    "
                    >{{ row.holiday || 0 }}</span
                  >
                </template>
              ...
              <el-table-column align="center">
                <template #header>
                  <span
                    class="cursor-pointer select-none"
                    :class="sortHeaderClass('total')"
                    @click="toggleSort('total')"
                    >合计 <span v-html="sortIconHtml('total')"></span
                  ></span>
                </template>
                <template #default="{ row }">
                  <span
              ...
              <el-table-column align="center" min-width="140">
                <template #header>
                  <span
                    class="cursor-pointer select-none"
                    :class="sortHeaderClass('freq')"
                    @click="toggleSort('freq')"
                    >{{ customFreqLabel }}
                    <span v-html="sortIconHtml('freq')"></span
                  ></span>
                </template>
                <template #default="{ row }">
        ...
        <div v-else class="card p-16 text-center">
          <el-icon size="40" class="text-gray-300 mb-3"
            ><DataAnalysis
          /></el-icon>
